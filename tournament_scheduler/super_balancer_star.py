...
days_dates = {
    1: '04/07/2026 (T7)', 2: '05/07/2026 (CN)', 3: '06/07/2026 (T2)', 4: '07/07/2026 (T3)', 5: '09/07/2026 (T5)',
...
# Map logical day (1..15) to July calendar day for rest calculation
cal_days = {
    1: 4, 2: 5, 3: 6, 4: 7, 5: 9,
...
for day, r, m1, m2 in schedule:
    cal_date = cal_days[day]
    date_str_full = days_dates[day]
    
    t1, t2 = m1[1]
    rest1_str = f"{cal_date - team_last_date[t1] - 1} ngày" if t1 in team_last_date else "-"
    rest2_str = f"{cal_date - team_last_date[t2] - 1} ngày" if t2 in team_last_date else "-"
    team_last_date[t1] = cal_date
    team_last_date[t2] = cal_date
    md += f"| {r} | {date_str_full} | 14:30 | {t1} | {t2} | {m1[0]} | {rest1_str} | {rest2_str} |\n"
    
    t3, t4 = m2[1]
    rest3_str = f"{cal_date - team_last_date[t3] - 1} ngày" if t3 in team_last_date else "-"
    rest4_str = f"{cal_date - team_last_date[t4] - 1} ngày" if t4 in team_last_date else "-"
